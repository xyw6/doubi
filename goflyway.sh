...
Service_goflyway(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xyw6/doubi/master/service/goflyway_centos -O /etc/init.d/goflyway; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xyw6/doubi/master/service/goflyway_debian -O /etc/init.d/goflyway; then
			echo -e "${Error} GoFlyway 服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/goflyway
